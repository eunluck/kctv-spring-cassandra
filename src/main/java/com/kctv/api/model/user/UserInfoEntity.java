...
public class UserInfoEntity implements UserDetails {
    ...
    public void modifyUser(UserInfoEntity userInfoEntity, String pwd){
        ...
        if (userInfoEntity.getAccept() != null && !userInfoEntity.getAccept().isEmpty())
            this.accept = userInfoEntity.getAccept();
        if (CollectionUtils.isNotEmpty(userInfoEntity.getUserMac()))
            this.userMac = userInfoEntity.getUserMac();
        if (!Strings.isNullOrEmpty(userInfoEntity.getUserNickname()))
            this.userNickname = userInfoEntity.getUserNickname();
        ...
        if (!Strings.isNullOrEmpty(userInfoEntity.getUserAddress()))
            this.userAddress = userInfoEntity.getUserAddress();
        if (!Strings.isNullOrEmpty(userInfoEntity.getUserGender()))
            this.userGender = userInfoEntity.getUserGender();
        if (!Strings.isNullOrEmpty(userInfoEntity.getUserBirth()))
            this.userBirth = userInfoEntity.getUserBirth();
            this.updateDate = new Date();
        if(CollectionUtils.isNotEmpty(userInfoEntity.getRoles()))
            this.roles = userInfoEntity.getRoles();
